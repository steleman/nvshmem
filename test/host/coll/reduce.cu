...
int main(int argc, char **argv) {
    ...
    while (get_next_team(&team)) {
        if (team != NVSHMEM_TEAM_INVALID) {
            ...
            for (int iter = 0; iter < MAX_ITERS; iter++) {
                NVSHMEMI_REPT_TYPES_AND_OPS_FOR_REDUCE(DO_REDUCE_TEST)
                NVSHMEMI_REPT_TYPES_AND_OPS_FOR_REDUCE(DO_REDUCE_ON_STREAM_TEST)
                COLL_CHECK_ERRS_D();
            }
            nvshmem_barrier_all();
        } else {
    ...
    for (nelems = 1; nelems <= MAX_ELEMS; nelems *= 2) {
        for (int iter = 0; iter < iters; iter++) {
            DO_REDUCE_TEST(double, double, sum)
            DO_REDUCE_ON_STREAM_TEST(double, double, sum)
        }
    }
    COLL_CHECK_ERRS_D();
    nvshmem_barrier_all();
