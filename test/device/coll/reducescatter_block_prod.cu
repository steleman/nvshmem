 ...
 */
#define CUMODULE_NAME "reducescatter_block_prod.cubin"
...
NVSHMEMTEST_REPT_FOR_BITWISE_REDUCE_TYPES_WITH_SCOPE2(DECL_TYPENAME_OP_REDUCESCATTER, block, _block,
                                                      x, prod)
    ...
    while (get_next_team(&team)) {
        if (team != NVSHMEM_TEAM_INVALID) {
            ...
            for (nelems = 1; nelems <= ELEMS_PER_THREAD * num_threads; nelems *= 2) {
                DEBUG_PRINT("block version, num elements = %ld\n", nelems);
