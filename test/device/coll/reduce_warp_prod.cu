 ...
 */
#define CUMODULE_NAME "reduce_warp_prod.cubin"
...
NVSHMEMTEST_REPT_FOR_ARITH_REDUCE_TYPES_WITH_SCOPE2(DECL_TYPENAME_OP_REDUCE, warp, _warp, x, prod)
...
NVSHMEMTEST_REPT_FOR_ARITH_REDUCE_TYPES_WITH_SCOPE2(DEFN_TYPENAME_OP_REDUCE, warp, _warp, x, prod)
#if defined __cplusplus || defined NVSHMEM_HOSTLIB_ONLY
}
#endif
    ...
    while (get_next_team(&team)) {
        if (team != NVSHMEM_TEAM_INVALID) {
            ...
            for (nelems = 1; nelems <= ELEMS_PER_THREAD * num_threads; nelems *= 2) {
                ...
                NVSHMEMTEST_REPT_FOR_ARITH_REDUCE_TYPES_WITH_SCOPE2(DO_RDXN_TEST, warp, _warp, x,
                                                                    prod)
            }
