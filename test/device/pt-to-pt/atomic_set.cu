 ...
 */
#define CUMODULE_NAME "atomic_set.cubin"
...
#define TEST_NVSHMEM_ATOMIC_SET_CUBIN(TYPENAME, TYPE, OP)                                    \
    void *args_##TYPENAME##_set_##OP[] = {(void *)&remote};                                  \
    CUfunction test_##TYPENAME##_set_##OP##_cubin;                                           \
    init_test_case_kernel(&test_##TYPENAME##_set_##OP##_cubin,                               \
                          NVSHMEMI_TEST_STRINGIFY(test_nvshmem_##TYPENAME##_##OP##_kernel)); \
    CU_CHECK(cuLaunchKernel(test_##TYPENAME##_set_##OP##_cubin, 1, 1, 1, 1, 1, 1, 0, 0,      \
                            args_##TYPENAME##_set_##OP, NULL));

#if defined __cplusplus || defined NVSHMEM_HOSTLIB_ONLY
extern "C" {
#endif

#define TEST_NVSHMEM_ATOMIC_SET_KERNEL(OP, TYPE, TYPENAME)                              \
    __global__ void test_nvshmem_##TYPENAME##_##OP##_kernel(TYPE *remote) {             \
        const int mype = nvshmem_my_pe();                                               \
        const int npes = nvshmem_n_pes();                                               \
        switch (OP) {                                                                   \
            case ATOMIC_SET:                                                            \
                nvshmem_##TYPENAME##_atomic_set(remote, (TYPE)mype, (mype + 1) % npes); \
                break;                                                                  \
            default:                                                                    \
...
REPT_MACRO_FOR_TYPES(TEST_NVSHMEM_ATOMIC_SET_KERNEL, ATOMIC_SET)

#if defined __cplusplus || defined NVSHMEM_HOSTLIB_ONLY
}
#endif

#define TEST_NVSHMEM_ATOMIC_SET(OP, TYPE, TYPENAME)                                         \
    do {                                                                                    \
        TYPE *remote;                                                                       \
        if (use_mmap) {                                                                     \
        ...
        nvshmem_barrier_all();                                                              \
        if (use_cubin) {                                                                    \
            TEST_NVSHMEM_ATOMIC_SET_CUBIN(TYPENAME, TYPE, OP);                              \
        } else {                                                                            \
...
int main(int argc, char *argv[]) {
    ...
    REPT_MACRO_FOR_TYPES(TEST_NVSHMEM_ATOMIC_SET, ATOMIC_SET)
