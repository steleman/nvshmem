 ...
 */
#define CUMODULE_NAME "put_nbi.cubin"
#include "nvshmem.h"
#include "nvshmemx.h"
#include "utils.h"
...
#define DEFINE_RMA_PUT_WRAPPER(Group)                                                  \
    __device__ void rma_put_wrapper_##Group(int *src, int *dest, size_t len, int pe) { \
        nvshmemx_int_put_nbi_##Group(dest, src, len, pe);                              \
    }
...
__device__ void rma_put_wrapper(int *src, int *dest, size_t len, int pe) {
    // nvshmem_int_put_nbi(dest, src, len, pe);
#ifdef __CUDA_ARCH__
    nvshmemx_qp_int_put_nbi(dest, src, len, pe, 0);
#endif
}
