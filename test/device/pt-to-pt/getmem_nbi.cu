 ...
 */
#define CUMODULE_NAME "getmem_nbi.cubin"
#include "nvshmem.h"
#include "nvshmemx.h"
#include "utils.h"
...
#define DEFINE_THREADGROUP_API(Group)                                                              \
    __global__ void alltoall_##Group(int *src, int *dest, size_t len, int mype, int npes) {        \
        for (int i = 0; i < npes; i++) {                                                           \
            nvshmemx_getmem_nbi_##Group((void *)(dest + (size_t)i * len),                          \
                                        (void *)(src + (size_t)mype * len), len * sizeof(int), i); \
        }                                                                                          \
        nvshmem_quiet();                                                                           \
    }                                                                                              \
                                                                                                   \
    __global__ void ring_##Group(int *src, int *dest, size_t len, int prevpe) {                    \
        nvshmemx_getmem_nbi_##Group((void *)dest, (void *)src, len * sizeof(int), prevpe);         \
        nvshmem_quiet();                                                                           \
    }                                                                                              \
                                                                                                   \
    ...
    void launch_ring_##Group(void *src, void *dest, size_t len, int nextpe, int prevpe,            \
        ...
        if (use_cubin) {                                                                           \
            ...
            ring_##Group<<<1, 1, 0, cstrm>>>(src_, dest_, len, prevpe);                            \
        }                                                                                          \
    }
...
__global__ void alltoall(int *src, int *dest, size_t len, int mype, int npes) {
    for (int i = 0; i < npes; i++) {
        nvshmem_getmem_nbi((void *)(dest + (size_t)i * len), (void *)(src + (size_t)mype * len),
                           len * sizeof(int), i);
    }
    nvshmem_quiet();
}

__global__ void ring(int *src, int *dest, size_t len, int prevpe) {
    nvshmem_getmem_nbi((void *)dest, (void *)src, len * sizeof(int), prevpe);
    nvshmem_quiet();
}
...
void launch_ring(void *src, void *dest, size_t len, int nextpe, int prevpe, cudaStream_t cstrm) {
    ...
    if (use_cubin) {
        ...
        ring<<<1, 1, 0, cstrm>>>(src_, dest_, len, prevpe);
    }
}
