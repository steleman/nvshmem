 ...
 */
#define CUMODULE_NAME "iput.cubin"
#include "nvshmem.h"
#include "nvshmemx.h"
#include "utils.h"
...
#define DEFINE_THREADGROUP_API(Group)                                                            \
    ...
    __global__ void alltoall_##Group(T *src, T *dest, int dstride, int sstride, size_t len,      \
        ...
        for (int i = 0; i < npes; i++) {                                                         \
            rma_iput_wrapper_##Group(src + (size_t)i * len * (size_t)sstride,                    \
                                     dest + (size_t)mype * len * (size_t)dstride, dstride,       \
                                     sstride, len, i);                                           \
        }                                                                                        \
                                                                                                 \
        nvshmem_quiet();                                                                         \
    ...
    __global__ void ring_##Group(T *src, T *dest, int dstride, int sstride, size_t len,          \
                                 int nextpe) {                                                   \
        rma_iput_wrapper_##Group(src, dest, dstride, sstride, len, nextpe);                      \
                                                                                                 \
        nvshmem_quiet();                                                                         \
    }                                                                                            \
                                                                                                 \
    ...
    void launch_ring_##Group(void *src, void *dest, size_t len, int nextpe, int prevpe,          \
        ...
        if (use_cubin) {                                                                         \
            ...
            ring_##Group<T><<<1, 1, 0, cstrm>>>(src_, dest_, ldisp, ldisp, len, nextpe);         \
        }                                                                                        \
    }
...
__device__ void rma_iput_wrapper(int *src, int *dest, int dstride, int sstride, size_t len,
                                 int pe) {
    nvshmem_int_iput(dest, src, dstride, sstride, len, pe);
}
...
__global__ void alltoall(T *src, T *dest, int dstride, int sstride, size_t len, int mype,
                         int npes) {
    for (int i = 0; i < npes; i++) {
        rma_iput_wrapper(src + (size_t)i * len * (size_t)sstride,
                         dest + (size_t)mype * len * (size_t)dstride, dstride, sstride, len, i);
    }
...
template <typename T>
__global__ void ring(T *src, T *dest, int dstride, int sstride, size_t len, int nextpe) {
...
__global__ void ring_int(int *src, int *dest, int dstride, int sstride, int len, int nextpe) {
    RING_TEMP();
}
...
void launch_ring(void *src, void *dest, size_t len, int nextpe, int prevpe, cudaStream_t cstrm) {
    ...
    if (use_cubin) {
        ...
        ring<T><<<1, 1, 0, cstrm>>>(src_, dest_, ldisp, ldisp, len, nextpe);
    }
}
