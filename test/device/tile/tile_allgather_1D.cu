...
#define V4_SRC_TILE_SIZE_0 16
#define V4_SRC_TILE_SIZE_1 1
#define V4_SRC_MATRIX_SHAPE 64  // should not overflow fp16, bfloat16
...
#define MAKE_V4_SRC_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});
#define MAKE_V4_DST_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});

#define V2_SRC_TILE_SIZE_0 14
#define V2_SRC_TILE_SIZE_1 1
#define V2_SRC_MATRIX_SHAPE 56  // should not overflow fp16, bfloat16
...
#define MAKE_V2_SRC_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});
#define MAKE_V2_DST_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});

#define V1_SRC_TILE_SIZE_0 14
#define V1_SRC_TILE_SIZE_1 1
#define V1_SRC_MATRIX_SHAPE 56  // should not overflow fp16, bfloat16
...
#define MAKE_V1_SRC_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});
#define MAKE_V1_DST_STRIDE nvshmemx::make_stride<ConstInt<1>>(ConstInt<1>{});
...
#define DEFN_TYPENAME_ALLGATHER(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, VLN)                 \
    ...
    __global__ void test_##TYPENAME##_tile_allgather_kernel_v##VLN##SC_SUFFIX(                     \
        ...
        auto dst_tile_shape = nvshmemx::make_shape(V##VLN##_SRC_TILE_SIZE_0 * npes);               \
        src_num_tiles_major = V##VLN##_SRC_TENSOR_SIZE_MAJOR / get<major_dim>(src_tile_shape);     \
        src_num_tiles_minor = 1;                                                                   \
        ...
        int team_id = (blockIdx.x * SC##s_per_block) + (threadIdx.x / nvshmtest_##SC##_size());    \
                                                                                                   \
        auto boundary = nvshmemx::make_shape(int(src_tensor_size_0));                              \
        /* grid is 1D */                                                                           \
        ...
        for (int i = blockIdx.x * SC##s_per_block; i < src_num_tiles_major * src_num_tiles_minor;  \
            ...
            size_t src_offset = (((my_tile_idx)*get<major_dim>(src_tile_shape) *                   \
                                  get<major_dim>(src_tile_stride)));                               \
            size_t dst_offset = (((my_tile_idx)*get<major_dim>(dst_tile_shape) *                   \
        ...
        for (int i = blockIdx.x * SC##s_per_block; i < src_num_tiles_major * src_num_tiles_minor;  \
            ...
            auto dest_tensor = nvshmemx::Tensor<TYPE, decltype(dst_tile_layout)>(dest_tile_start,  \
                                                                                 dst_tile_layout); \
            validate_##TYPENAME##_tile_allgather_1D_data##SC_SUFFIX<                               \
...
#define DO_ALLGATHER_TEST(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, VLN)                       \
    init_##TYPENAME##_tile_allgather_data_kernel<<<8, 256, 0, cstrm>>>(                            \
        ...
        V##VLN##_SRC_MATRIX_SHAPE /* src size 0*/, 1 /* src size 1*/, V##VLN##_SRC_TILE_SIZE_0,    \
        V##VLN##_SRC_TILE_SIZE_1, 1 /* dstStride major*/, 0 /* dstStride minor*/,                  \
        1 /*along major_dim*/);                                                                    \
    ...
    test_##TYPENAME##_tile_allgather_kernel_v##VLN##SC_SUFFIX<<<grid_size, block_size, 0,          \
        ...
        1 /* src tensor size 1*/, V##VLN##_SRC_MATRIX_SHAPE * npes /* dst tensor size 0*/,         \
