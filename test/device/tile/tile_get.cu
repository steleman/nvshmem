 ...
 */
 ...
 #define DECL_TYPENAME_TILE_GET(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE)                     \
     __global__ void test_##TYPENAME##_tile_get_kernel##SC_SUFFIX(                    \
         nvshmem_team_t *teams_dev, TYPE *dest, TYPE *source, size_t nelems, size_t tensor_size_0, \
         size_t tensor_size_1, int npes);

 NVSHMEMTEST_TILE_REPT_TYPES_AND_SCOPES(DECL_TYPENAME_TILE_GET, NA)
 #undef DECL_TYPENAME_TILE_GET

#define VALIDATE_TILE_GET_DATA(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE)               \
    ...
    __device__ void validate_##TYPENAME##_copy_tile_data##SC_SUFFIX(                 \
        ...
        for (size_t i = myIdx; i < tot_elem_in_tile; i = i + groupSize) {                        \
            ...
            if constexpr (std::is_same_v<TYPE, half>) {                                          \
                /* data is received from my_pe-1; 2049 is beyond half precision range */         \
                ...
                if (!my_pe) {                                                                    \
                    /* 257 is beyond bfloat16 precision range for this test */                   \
...
NVSHMEMTEST_TILE_REPT_TYPES_AND_SCOPES(VALIDATE_TILE_GET_DATA, NA)
#undef VALIDATE_TILE_GET_DATA


 #define DEFN_TYPENAME_TILE_GET(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, VLN)                \
     /* This is a persistent kernel which iterated over all the tiles */                           \
     /* Both Grid and block are 1D */                                                              \
     __global__ void test_##TYPENAME##_tile_get_kernel_v##VLN##SC_SUFFIX(                       \
         nvshmem_team_t *teams_dev, TYPE *dest, TYPE *source, size_t nelems, size_t tensor_size_0, \
 ...
 NVSHMEMTEST_TILE_REPT_SCOPES_AND_VLEN(DEFN_TYPENAME_TILE_GET, NA, float, float)
 NVSHMEMTEST_TILE_REPT_SCOPES_AND_VLEN(DEFN_TYPENAME_TILE_GET, NA, half, half)
 NVSHMEMTEST_TILE_REPT_SCOPES_AND_VLEN(DEFN_TYPENAME_TILE_GET, NA, bfloat16, __nv_bfloat16)

 #define DO_GET_TEST(OP, SC, SC_SUFFIX, SC_PREFIX, TYPENAME, TYPE, VLN)                             \
     ...
     test_##TYPENAME##_tile_get_kernel_v##VLN##SC_SUFFIX<<<grid_size, block_size, 0,                \
                                                                        cstrm>>>(                   \
 ...
 int main(int argc, char **argv) {
     ...
     if (threadgroup_scope.type == NVSHMEM_THREAD || threadgroup_scope.type == NVSHMEM_ALL_SCOPES) {
         ...
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, thread, , , float, float)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, thread, , , half, half)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, thread, , , bfloat16, __nv_bfloat16)
     }
     ...
     if (threadgroup_scope.type == NVSHMEM_WARP || threadgroup_scope.type == NVSHMEM_ALL_SCOPES) {
         ...
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warp, _warp, x, float, float)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warp, _warp, x, half, half)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warp, _warp, x, bfloat16, __nv_bfloat16)
     }
     ...
     if (threadgroup_scope.type == NVSHMEM_WARPGROUP || threadgroup_scope.type == NVSHMEM_ALL_SCOPES) {
         ...
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warpgroup, _warpgroup, x, float, float)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warpgroup, _warpgroup, x, half, half)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, warpgroup, _warpgroup, x, bfloat16,
         __nv_bfloat16)
     }
     ...
     if (threadgroup_scope.type == NVSHMEM_BLOCK || threadgroup_scope.type == NVSHMEM_ALL_SCOPES) {
         ...
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, block, _block, x, float, float)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, block, _block, x, half, half)
         NVSHMEMTEST_TILE_REPT_VLEN(DO_GET_TEST, NA, block, _block, x, bfloat16, __nv_bfloat16)
     }
