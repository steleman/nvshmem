...
class nvshmemi_symmetric_heap {
   public:
    ...
    virtual ~nvshmemi_symmetric_heap() = default;
    ...
    void *heap_calloc(size_t size, size_t count);
    void *heap_align(size_t size, size_t alignment);
   ...
   protected:
    ...
    virtual int update_heap_handle_cache(void *buf, size_t size);
    /**
    ...
    void *allocate_virtual_memory_from_mspace(size_t size, size_t count, size_t alignment,
                                              int type);
    nvshmemi_state_t *state_;  // store a reference of device state instance
    CUmemAllocationHandleType mem_handle_type_ = CU_MEM_HANDLE_TYPE_NONE;
    size_t mem_granularity_;
    size_t log2_mem_granularity_;
    size_t physical_heap_size_;
    size_t heap_size_;
    uint64_t reserved_heap_size_ = 0;
    void *global_heap_base_;
    void *heap_base_;
    void **peer_heap_base_remote_;
    void **peer_heap_base_p2p_;
    int heap_handle_cache_ = 0;
    nvshmemi_mem_remote_transport *remote_ref_;  // holds an instance of remote abstraction
    nvshmemi_mem_p2p_transport *p2p_ref_;        // holds an instance of memp2p abstraction
    mspace *heap_mspace_;
...
inline nvshmem_mem_handle *nvshmemi_symmetric_heap::get_transport_mem_handle(void *addr,
    ...
    return &remote_handles_[handle_idx][handle_sub_index];
}
