...
/*@
SPMI_Bcast - broadcast from root process to all peers in the process group

Input Parameters:
- buffer - A pointer to a valid message buffer
- length - Length of the message buffer

Return values:
+ SPMI_SUCCESS - broadcast successfully finished
- SPMI_FAIL - broadcast failed

Notes:
This function is a collective call across all processes in the process group
the local process belongs to.  It will not return until all the processes
have called 'SPMI_Bcast'. The root process is assumed to be rank 0.
